...
impl Upstream {
    ...
    async fn setup(mut connection: UpstreamConnection) -> Result<Arc<Mutex<Self>>, ()> {
        ...
        let open_channel_message: StdFrame = PoolMessages::Mining(
            Mining::OpenExtendedMiningChannel(Self::get_open_extended_channel()),
        )
        .try_into()
        .unwrap();
        let sv2_frame = open_channel_message.into();
        let sender = self_.safe_lock(|s| s.connection.sender.clone()).unwrap();
    ...
    fn get_open_extended_channel() -> OpenExtendedMiningChannel<'static> {
        OpenExtendedMiningChannel {
            request_id: 0.into(),
            user_identity: String::new().try_into().unwrap(),
            nominal_hash_rate: 0.0,
            max_target: u256_from_int(567_u64),
            min_extranonce_size: 100,
        }
    }
}
